...
inline ImGuiTableFlags TableFixFlags(ImGuiTableFlags flags, ImGuiWindow* outer_window)
{
    ...
    if ((flags & ImGuiTableFlags_SizingMask_) == 0)
        flags |= ((flags & ImGuiTableFlags_ScrollX) or (outer_window->Flags & ImGuiWindowFlags_AlwaysAutoResize)) ? ImGuiTableFlags_SizingFixedFit : ImGuiTableFlags_SizingStretchSame;
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    const bool outer_window_is_measuring_size = (outer_window->AutoFitFramesX > 0) or (outer_window->AutoFitFramesY > 0); // Doesn't apply to AlwaysAutoResize windows!
    if (use_child_window and IsClippedEx(outer_rect, 0) and !outer_window_is_measuring_size)
    {
        ItemSize(outer_rect);
        return false;
    }
    ...
    if (instance_no > 0)
    {
        IM_ASSERT(table->ColumnsCount == columns_count and "BeginTable(): Cannot change columns count mid-frame while preserving same ID");
    ...
    if (use_child_window)
    {
        ...
        if ((flags & ImGuiTableFlags_ScrollX) and !(flags & ImGuiTableFlags_ScrollY))
        ...
        if ((flags & ImGuiTableFlags_ScrollX) and inner_width > 0.0f)
            override_content_size.x = inner_width;

        if (override_content_size.x != FLT_MAX or override_content_size.y != FLT_MAX)
        ...
        IM_ASSERT(table->InnerWindow->WindowPadding.x == 0.0f and table->InnerWindow->WindowPadding.y == 0.0f and table->InnerWindow->WindowBorderSize == 0.0f);

        // Allow submitting when host is measuring
        if (table->InnerWindow->SkipItems and outer_window_is_measuring_size)
    ...
    const float inner_spacing_explicit = (pad_inner_x and (flags & ImGuiTableFlags_BordersInnerV) == 0) ? g.Style.CellPadding.x : 0.0f;
    const float inner_padding_explicit = (pad_inner_x and (flags & ImGuiTableFlags_BordersInnerV) != 0) ? g.Style.CellPadding.x : 0.0f;
    ...
    if ((table_last_flags & ImGuiTableFlags_Reorderable) and (flags & ImGuiTableFlags_Reorderable) == 0)
    ...
    if (old_columns_count != 0 and old_columns_count != columns_count)
    {
    ...
    if (table->IsInitializing)
    {
        ...
        for (int n = 0; n < columns_count; n++)
        {
            ...
            if (old_columns_to_preserve and n < old_columns_count)
            {
    ...
    if (table->RefScale != 0.0f and table->RefScale != new_ref_scale_unit)
    {
...
void ImGui::TableBeginApplyRequests(ImGuiTable* table)
{
    ...
    {
        if (table->ResizedColumn != -1 and table->ResizedColumnNextWidth != FLT_MAX)
    ...
    if (table->InstanceCurrent == 0)
    {
        if (table->HeldHeaderColumn == -1 and table->ReorderColumn != -1)
            table->ReorderColumn = -1;
        table->HeldHeaderColumn = -1;
        if (table->ReorderColumn != -1 and table->ReorderColumnDir != 0)
        {
            ...
            IM_ASSERT(reorder_dir == -1 or reorder_dir == +1);
...
static void TableSetupColumnFlags(ImGuiTable* table, ImGuiTableColumn* column, ImGuiTableColumnFlags flags_in)
{
    ...
    if ((flags & ImGuiTableColumnFlags_WidthMask_) == 0)
    {
        ...
        if (table_sizing_policy == ImGuiTableFlags_SizingFixedFit or table_sizing_policy == ImGuiTableFlags_SizingFixedSame)
    ...
    // Sorting
    if ((flags & ImGuiTableColumnFlags_NoSortAscending) and (flags & ImGuiTableColumnFlags_NoSortDescending))
    ...
    if (table->Flags & ImGuiTableFlags_Sortable)
    {
        ...
        if ((flags & ImGuiTableColumnFlags_PreferSortAscending)  != 0 and (flags & ImGuiTableColumnFlags_NoSortAscending)  == 0) { mask |= 1 << ImGuiSortDirection_Ascending;  list |= ImGuiSortDirection_Ascending  << (count << 1); count++; }
        if ((flags & ImGuiTableColumnFlags_PreferSortDescending) != 0 and (flags & ImGuiTableColumnFlags_NoSortDescending) == 0) { mask |= 1 << ImGuiSortDirection_Descending; list |= ImGuiSortDirection_Descending << (count << 1); count++; }
        if ((flags & ImGuiTableColumnFlags_PreferSortAscending)  == 0 and (flags & ImGuiTableColumnFlags_NoSortAscending)  == 0) { mask |= 1 << ImGuiSortDirection_Ascending;  list |= ImGuiSortDirection_Ascending  << (count << 1); count++; }
        if ((flags & ImGuiTableColumnFlags_PreferSortDescending) == 0 and (flags & ImGuiTableColumnFlags_NoSortDescending) == 0) { mask |= 1 << ImGuiSortDirection_Descending; list |= ImGuiSortDirection_Descending << (count << 1); count++; }
        if ((table->Flags & ImGuiTableFlags_SortTristate) or count == 0) { mask |= 1 << ImGuiSortDirection_None; count++; }
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        if (!(table->Flags & ImGuiTableFlags_Hideable) or (column->Flags & ImGuiTableColumnFlags_NoHide))
        ...
        column->IsEnabled = column->IsUserEnabled and (column->Flags & ImGuiTableColumnFlags_Disabled) == 0;

        if (column->SortOrder != -1 and !column->IsEnabled)
            table->IsSortSpecsDirty = true;
        if (column->SortOrder > 0 and !(table->Flags & ImGuiTableFlags_SortMulti))
            table->IsSortSpecsDirty = true;
        ...
        if (column_is_resizable)
            has_resizable = true;
        if ((column->Flags & ImGuiTableColumnFlags_WidthFixed) and column->InitStretchWeightOrWidth > 0.0f and !column_is_resizable)
    ...
    if ((table->Flags & ImGuiTableFlags_Sortable) and table->SortSpecsCount == 0 and !(table->Flags & ImGuiTableFlags_SortTristate))
        table->IsSortSpecsDirty = true;
    table->RightMostEnabledColumn = (ImGuiTableColumnIdx)prev_visible_column_idx;
    IM_ASSERT(table->LeftMostEnabledColumn >= 0 and table->RightMostEnabledColumn >= 0);
    ...
    if (has_auto_fit_request and table->OuterWindow != table->InnerWindow)
        table->InnerWindow->SkipItems = false;
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        if (column->Flags & ImGuiTableColumnFlags_WidthFixed)
        {
            ...
            float width_auto = column->WidthAuto;
            if (table_sizing_policy == ImGuiTableFlags_SizingFixedSame and (column->AutoFitQueue != 0x00 or !column_is_resizable))
            ...
            else if ((column->Flags & ImGuiTableColumnFlags_WidthFixed) and !column_is_resizable and column->IsRequestOutput)
                column->WidthRequest = width_auto;
            ...
            if (column->AutoFitQueue > 0x01 and table->IsInitializing and !column->IsPreserveWidthAuto)
        ...
        else
        {
            ...
            if (column->AutoFitQueue != 0x00 or column->StretchWeight < 0.0f or !column_is_resizable)
            {
            ...
            if (table->LeftMostStretchedColumn == -1 or table->Columns[table->LeftMostStretchedColumn].DisplayOrder > column->DisplayOrder)
                table->LeftMostStretchedColumn = (ImGuiTableColumnIdx)column_n;
            if (table->RightMostStretchedColumn == -1 or table->Columns[table->RightMostStretchedColumn].DisplayOrder < column->DisplayOrder)
    ...
    const float width_removed = (table->HasScrollbarYPrev and !table->InnerWindow->ScrollbarY) ? g.Style.ScrollbarSize : 0.0f; // To synchronize decoration width of synched tables with mismatching scrollbar state (#5920)
    const float width_avail = ImMax(1.0f, (((table->Flags & ImGuiTableFlags_ScrollX) and table->InnerWidth == 0.0f) ? table->InnerClipRect.GetWidth() : work_rect.GetWidth()) - width_removed);
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        if (column->NextEnabledColumn == -1 and table->LeftMostStretchedColumn != -1)
    ...
    if (width_remaining_for_stretched_columns >= 1.0f and !(table->Flags & ImGuiTableFlags_PreciseWidths))
        for (int order_n = table->ColumnsCount - 1; stretch_sum_weights > 0.0f and width_remaining_for_stretched_columns >= 1.0f and order_n >= 0; order_n--)
        {
    ...
    if (table->AngledHeadersHeight > 0.0f)
        if (g.IO.MousePos.y >= table->OuterRect.Min.y and g.IO.MousePos.y <= table->OuterRect.Min.y + table->AngledHeadersHeight)
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        if (offset_x_frozen and table->FreezeColumnsCount == visible_n)
        {
        ...
        if (is_hovering_table and mouse_skewed_x >= column->ClipRect.Min.x and mouse_skewed_x < column->ClipRect.Max.x)
        ...
        column->IsSkipItems = !column->IsEnabled or table->HostSkipItems;
        if (column->IsSkipItems)
            IM_ASSERT(!is_visible);
        if (column->IsRequestOutput and !column->IsSkipItems)
    ...
    if (is_hovering_table and table->HoveredColumnBody == -1)
        if (mouse_skewed_x >= unused_x1)
            table->HoveredColumnBody = (ImGuiTableColumnIdx)table->ColumnsCount;
    if (has_resizable == false and (table->Flags & ImGuiTableFlags_Resizable))
    ...
    if (table->IsContextPopupOpen and table->ContextPopupColumn != -1 and table->InstanceInteracted == table->InstanceCurrent)
        table->HighlightColumnHeader = table->ContextPopupColumn;
    else if ((table->Flags & ImGuiTableFlags_HighlightHoveredColumn) and table->HoveredColumnBody != -1 and table->HoveredColumnBody != table->ColumnsCount and table->HoveredColumnBorder == -1)
        if (g.ActiveId == 0 or (table->IsActiveIdInTable or g.DragDropActive))
    ...
    if (table->DisableDefaultContextMenu == false and TableBeginContextMenuPopup(table))
    {
    ...
    if (table->IsSortSpecsDirty and (table->Flags & ImGuiTableFlags_Sortable))
...
void ImGui::TableUpdateBorders(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        if ((table->Flags & ImGuiTableFlags_NoBordersInBody) and table->IsUsingHeaders == false)
            continue;

        if (!column->IsVisibleX and table->LastResizedColumn != column_n)
            continue;
        ...
        if (pressed and IsMouseDoubleClicked(0))
        {
        ...
        if ((hovered and g.HoveredIdTimer > TABLE_RESIZE_SEPARATOR_FEEDBACK_TIMER) or held)
        {
...
void    ImGui::EndTable()
{
    ...
    IM_ASSERT(table != NULL and "Only call EndTable() if BeginTable() returns true!");

    // This assert would be very useful to catch a common error... unfortunately it would probably trigger in some
    // cases, and for consistency user may sometimes output empty tables (and still benefit from e.g. outer border)
    //IM_ASSERT(table->IsLayoutLocked and "Table unused: never called TableNextRow(), is that the intent?");
    ...
    if (flags & ImGuiTableFlags_ContextMenuInBody)
        if (table->HoveredColumnBody != -1 and !IsAnyItemHovered() and IsMouseReleased(ImGuiMouseButton_Right))
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
        ...
        {
            ImGuiTableColumn* column = &table->Columns[column_n];
            float column_width_request = ((column->Flags & ImGuiTableColumnFlags_WidthFixed) and !(column->Flags & ImGuiTableColumnFlags_NoResize)) ? column->WidthRequest : TableGetColumnWidthAuto(table, column);
            if (column->Flags & ImGuiTableColumnFlags_WidthFixed)
                auto_fit_width_for_fixed += column_width_request;
            else
                auto_fit_width_for_stretched += column_width_request;
            if ((column->Flags & ImGuiTableColumnFlags_WidthStretch) and (column->Flags & ImGuiTableColumnFlags_NoResize) != 0)
    ...
    // Update scroll
    if ((table->Flags & ImGuiTableFlags_ScrollX) == 0 and inner_window != outer_window)
    {
        inner_window->Scroll.x = 0.0f;
    }
    else if (table->LastResizedColumn != -1 and table->ResizedColumn == -1 and inner_window->ScrollbarX and table->InstanceInteracted == table->InstanceCurrent)
    {
    ...
    if (table->ResizedColumn != -1 and table->InstanceCurrent == table->InstanceInteracted)
    {
    ...
    table->IsActiveIdInTable = (g.ActiveIdIsAlive != 0 and table->IsActiveIdAliveBeforeTable == false);
    ...
    IM_ASSERT(g.CurrentWindow == outer_window and g.CurrentTable == table);
...
void ImGui::TableSetupColumn(const char* label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    IM_ASSERT(table != NULL and "Need to call TableSetupColumn() after BeginTable()!");
    IM_ASSERT(table->IsLayoutLocked == false and "Need to call call TableSetupColumn() before first row!");
    IM_ASSERT((flags & ImGuiTableColumnFlags_StatusMask_) == 0 and "Illegal to pass StatusMask values to TableSetupColumn()");
    ...
    if (table->IsDefaultSizingPolicy and (flags & ImGuiTableColumnFlags_WidthMask_) == 0 and (flags & ImGuiTableFlags_ScrollX) == 0)
        IM_ASSERT(init_width_or_weight <= 0.0f and "Can only specify width/weight if sizing policy is set explicitly in either Table or Column.");

    // When passing a width automatically enforce WidthFixed policy
    // (whereas TableSetupColumnFlags would default to WidthAuto if table is not Resizable)
    if ((flags & ImGuiTableColumnFlags_WidthMask_) == 0 and init_width_or_weight > 0.0f)
        if ((table->Flags & ImGuiTableFlags_SizingMask_) == ImGuiTableFlags_SizingFixedFit or (table->Flags & ImGuiTableFlags_SizingMask_) == ImGuiTableFlags_SizingFixedSame)
            flags |= ImGuiTableColumnFlags_WidthFixed;
    ...
    if (table->IsInitializing)
    {
        ...
        if (column->WidthRequest < 0.0f and column->StretchWeight < 0.0f)
        {
            if ((flags & ImGuiTableColumnFlags_WidthFixed) and init_width_or_weight > 0.0f)
        ...
        if ((flags & ImGuiTableColumnFlags_DefaultHide) and (table->SettingsLoadedFlags & ImGuiTableFlags_Hideable) == 0)
            column->IsUserEnabled = column->IsUserEnabledNextFrame = false;
        if (flags & ImGuiTableColumnFlags_DefaultSort and (table->SettingsLoadedFlags & ImGuiTableFlags_Sortable) == 0)
        {
...
void ImGui::TableSetupScrollFreeze(int columns, int rows)
{
    ...
    ImGuiTable* table = g.CurrentTable;
    IM_ASSERT(table != NULL and "Need to call TableSetupColumn() after BeginTable()!");
    IM_ASSERT(table->IsLayoutLocked == false and "Need to call TableSetupColumn() before first row!");
    ...
    for (int column_n = 0; column_n < table->FreezeColumnsRequest; column_n++)
    {
        ...
        if (order_n != column_n and order_n >= table->FreezeColumnsRequest)
        {
...
{
    if (table->IsLayoutLocked == false and column_n >= table->DeclColumnsCount)
...
void ImGui::TableSetColumnEnabled(int column_n, bool enabled)
{
    ...
    if (column_n < 0)
        column_n = table->CurrentColumn;
    IM_ASSERT(column_n >= 0 and column_n < table->ColumnsCount);
    ImGuiTableColumn* column = &table->Columns[column_n];
...
void ImGui::TableSetBgColor(ImGuiTableBgTarget target, ImU32 color, int column_n)
{
    ...
    case ImGuiTableBgTarget_CellBg:
    {
        ...
        if (!IM_BITARRAY_TESTBIT(table->VisibleMaskByIndex, column_n))
            return;
        if (table->RowCellDataCurrent < 0 or table->RowCellData[table->RowCellDataCurrent].Column != column_n)
...
void ImGui::TableBeginRow(ImGuiTable* table)
{
    ...
    if (table->CurrentRow == 0 and table->FreezeRowsCount > 0)
...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    ImGuiTableInstanceData* table_instance = TableGetInstanceData(table, table->InstanceCurrent);
    if ((table->RowFlags & ImGuiTableRowFlags_Headers) and (table->CurrentRow == 0 or (table->LastRowFlags & ImGuiTableRowFlags_Headers)))
        table_instance->LastTopHeadersRowHeight += bg_y2 - bg_y1;

    const bool is_visible = (bg_y2 >= table->InnerClipRect.Min.y and bg_y1 <= table->InnerClipRect.Max.y);
    if (is_visible)
    {
        // Update data for TableGetHoveredRow()
        if (table->HoveredColumnBody != -1 and g.IO.MousePos.y >= bg_y1 and g.IO.MousePos.y < bg_y2 and table_instance->HoveredRowNext < 0)
        ...
        if (table->CurrentRow > 0 and (table->Flags & ImGuiTableFlags_BordersInnerH))
        ...
        if ((bg_col0 | bg_col1 | top_border_col) != 0 or draw_strong_bottom_border or draw_cell_bg_color)
        {
        ...
        if (bg_col0 or bg_col1)
        {
            ImRect row_rect(table->WorkRect.Min.x, bg_y1, table->WorkRect.Max.x, bg_y2);
            row_rect.ClipWith(table->BgClipRect);
            if (bg_col0 != 0 and row_rect.Min.y < row_rect.Max.y)
                window->DrawList->AddRectFilled(row_rect.Min, row_rect.Max, bg_col0);
            if (bg_col1 != 0 and row_rect.Min.y < row_rect.Max.y)
        ...
        // Draw top border
        if (top_border_col and bg_y1 >= table->BgClipRect.Min.y and bg_y1 < table->BgClipRect.Max.y)
            window->DrawList->AddLine(ImVec2(table->BorderX1, bg_y1), ImVec2(table->BorderX2, bg_y1), top_border_col, border_size);

        // Draw bottom border at the row unfreezing mark (always strong)
        if (draw_strong_bottom_border and bg_y2 >= table->BgClipRect.Min.y and bg_y2 < table->BgClipRect.Max.y)
...
bool ImGui::TableSetColumnIndex(int column_n)
{
    ...
    if (table->CurrentColumn != column_n)
    {
        if (table->CurrentColumn != -1)
            TableEndCell(table);
        IM_ASSERT(column_n >= 0 and table->ColumnsCount);
...
bool ImGui::TableNextColumn()
{
    ...
    if (table->IsInsideRow and table->CurrentColumn + 1 < table->ColumnsCount)
    {
        if (table->CurrentColumn != -1)
            TableEndCell(table);
...
void ImGui::TableBeginCell(ImGuiTable* table, int column_n)
{
    ...
    // Logging
    if (g.LogEnabled and !column->IsSkipItems)
    {
...
float ImGui::TableGetColumnWidthAuto(ImGuiTable* table, ImGuiTableColumn* column)
{
    ...
    if ((column->Flags & ImGuiTableColumnFlags_WidthFixed) and column->InitStretchWeightOrWidth > 0.0f)
        if (!(table->Flags & ImGuiTableFlags_Resizable) or (column->Flags & ImGuiTableColumnFlags_NoResize))
...
void ImGui::TableSetColumnWidth(int column_n, float width)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    IM_ASSERT(table != NULL and table->IsLayoutLocked == false);
    IM_ASSERT(column_n >= 0 and column_n < table->ColumnsCount);
    ...
    if (column_0->WidthGiven == column_0_width or column_0->WidthRequest == column_0_width)
        return;
    ...
    if (column_0->Flags & ImGuiTableColumnFlags_WidthFixed)
        if (!column_1 or table->LeftMostStretchedColumn == -1 or table->Columns[table->LeftMostStretchedColumn].DisplayOrder >= column_0->DisplayOrder)
        {
    ...
    IM_ASSERT(column_0_width > 0.0f and column_1_width > 0.0f);
    column_0->WidthRequest = column_0_width;
...
void ImGui::TableSetColumnWidthAutoAll(ImGuiTable* table)
{
    ...
    {
        ImGuiTableColumn* column = &table->Columns[column_n];
        if (!column->IsEnabled and !(column->Flags & ImGuiTableColumnFlags_WidthStretch)) // Cannot reset weight of hidden stretch column
            continue;
        column->CannotSkipItemsQueue = (1 << 0);
...
{
    IM_ASSERT(table->LeftMostStretchedColumn != -1 and table->RightMostStretchedColumn != -1);
    ...
    {
        ImGuiTableColumn* column = &table->Columns[column_n];
        if (!column->IsEnabled or !(column->Flags & ImGuiTableColumnFlags_WidthStretch))
            continue;
        IM_ASSERT(column->StretchWeight > 0.0f);
        visible_weight += column->StretchWeight;
        visible_width += column->WidthRequest;
    }
    IM_ASSERT(visible_weight > 0.0f and visible_width > 0.0f);
...
// - FreezeRows or FreezeColunns  --> 3+D+N*2 (unless scrolling value is zero)
...
void ImGui::TableSetupDrawChannels(ImGuiTable* table)
{
    ...
    const int channels_for_dummy = (table->ColumnsEnabledCount < table->ColumnsCount or (memcmp(table->VisibleMaskByIndex, table->EnabledMaskByIndex, ImBitArrayGetStorageSizeInBytes(table->ColumnsCount)) != 0)) ? +1 : 0;
    ...
    {
        ImGuiTableColumn* column = &table->Columns[column_n];
        if (column->IsVisibleX and column->IsVisibleY)
        {
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        for (int merge_group_sub_n = 0; merge_group_sub_n < merge_group_sub_count; merge_group_sub_n++)
        {
            ...
            if (src_channel->_CmdBuffer.Size > 0 and src_channel->_CmdBuffer.back().ElemCount == 0 and src_channel->_CmdBuffer.back().UserCallback == NULL) // Equivalent of PopUnusedDrawCmd()
            ...
            const int merge_group_n = (has_freeze_h and column_n < table->FreezeColumnsCount ? 0 : 1) + (has_freeze_v and merge_group_sub_n == 0 ? 0 : 2);
    ...
    if (merge_group_mask != 0)
    {
        ...
        IM_ASSERT(has_freeze_v == false or table->Bg2DrawChannelUnfrozen != TABLE_DRAW_CHANNEL_BG2_FROZEN);
        ...
        for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount)
            {
                ...
                if ((merge_group_n & 1) == 0 or !has_freeze_h)
                    merge_clip_rect.Min.x = ImMin(merge_clip_rect.Min.x, host_rect.Min.x);
                if ((merge_group_n & 2) == 0 or !has_freeze_v)
                    merge_clip_rect.Min.y = ImMin(merge_clip_rect.Min.y, host_rect.Min.y);
                if ((merge_group_n & 1) != 0)
                    merge_clip_rect.Max.x = ImMax(merge_clip_rect.Max.x, host_rect.Max.x);
                if ((merge_group_n & 2) != 0 and (table->Flags & ImGuiTableFlags_NoHostExtendY) == 0)
                ...
                for (int n = 0; n < splitter->_Count and merge_channels_count != 0; n++)
                {
                    ...
                    IM_ASSERT(channel->_CmdBuffer.Size == 1 and merge_clip_rect.Contains(ImRect(channel->_CmdBuffer[0].ClipRect)));
            ...
            if (merge_group_n == 1 and has_freeze_v)
                memcpy(dst_tmp++, &splitter->_Channels[table->Bg2DrawChannelUnfrozen], sizeof(ImDrawChannel));
        }

        // Append unmergeable channels that we didn't reorder at the end of the list
        for (int n = 0; n < splitter->_Count and remaining_count != 0; n++)
        {
...
static ImU32 TableGetColumnBorderCol(ImGuiTable* table, int order_n, int column_n)
{
    ...
    const bool is_resized = (table->ResizedColumn == column_n) and (table->InstanceInteracted == table->InstanceCurrent);
    const bool is_frozen_separator = (table->FreezeColumnsCount == order_n + 1);
    if (is_resized or is_hovered)
        return ImGui::GetColorU32(is_resized ? ImGuiCol_SeparatorActive : ImGuiCol_SeparatorHovered);
    if (is_frozen_separator or (table->Flags & (ImGuiTableFlags_NoBordersInBody | ImGuiTableFlags_NoBordersInBodyUntilResize)))
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    if (table->Flags & ImGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            const bool is_frozen_separator = (table->FreezeColumnsCount == order_n + 1);
            if (column->MaxX > table->InnerClipRect.Max.x and !is_resized)
                continue;

            // Decide whether right-most column is visible
            if (column->NextEnabledColumn == -1 and !is_resizable)
                if ((table->Flags & ImGuiTableFlags_SizingMask_) != ImGuiTableFlags_SizingFixedSame or (table->Flags & ImGuiTableFlags_NoHostExtendX))
                    continue;
            ...
            float draw_y2 = (is_hovered or is_resized or is_frozen_separator or (table->Flags & (ImGuiTableFlags_NoBordersInBody | ImGuiTableFlags_NoBordersInBodyUntilResize)) == 0) ? draw_y2_body : draw_y2_head;
            if (draw_y2 > draw_y1)
    ...
    if ((table->Flags & ImGuiTableFlags_BordersInnerH) and table->RowPosY2 < table->OuterRect.Max.y)
    {
        // Draw bottom-most row border between it is above outer border.
        const float border_y = table->RowPosY2;
        if (border_y >= table->BgClipRect.Min.y and border_y < table->BgClipRect.Max.y)
...
{
    if (column->SortOrder == -1 or (column->SortDirectionsAvailMask & (1 << column->SortDirection)) != 0)
        return;
...
IM_STATIC_ASSERT(ImGuiSortDirection_None == 0 and ImGuiSortDirection_Ascending == 1 and ImGuiSortDirection_Descending == 2);
...
void ImGui::TableSetColumnSortDirection(int column_n, ImGuiSortDirection sort_direction, bool append_to_sort_specs)
{
    ...
    if (column->SortDirection == ImGuiSortDirection_None)
        column->SortOrder = -1;
    else if (column->SortOrder == -1 or !append_to_sort_specs)
        column->SortOrder = append_to_sort_specs ? sort_order_max + 1 : 0;

    for (int other_column_n = 0; other_column_n < table->ColumnsCount; other_column_n++)
    {
        ImGuiTableColumn* other_column = &table->Columns[other_column_n];
        if (other_column != column and !append_to_sort_specs)
...
void ImGui::TableSortSpecsSanitize(ImGuiTable* table)
{
    ...
    const bool need_fix_single_sort_order = (sort_order_count > 1) and !(table->Flags & ImGuiTableFlags_SortMulti);
    if (need_fix_linearize or need_fix_single_sort_order)
    {
        ...
        for (int sort_n = 0; sort_n < sort_order_count; sort_n++)
        {
            ...
            for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
                if ((fixed_mask & ((ImU64)1 << (ImU64)column_n)) == 0 and table->Columns[column_n].SortOrder != -1)
                    if (column_with_smallest_sort_order == -1 or table->Columns[column_n].SortOrder < table->Columns[column_with_smallest_sort_order].SortOrder)
    ...
    if (sort_order_count == 0 and !(table->Flags & ImGuiTableFlags_SortTristate))
        for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
        {
            ImGuiTableColumn* column = &table->Columns[column_n];
            if (column->IsEnabled and !(column->Flags & ImGuiTableColumnFlags_NoSort))
            {
                sort_order_count = 1;
                column->SortOrder = 0;
                column->SortDirection = (ImU8)TableGetColumnAvailSortDirection(column, 0);
...
void ImGui::TableHeadersRow()
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    IM_ASSERT(table != NULL and "Need to call TableHeadersRow() after BeginTable()!");
    ...
    if (IsMouseReleased(1) and TableGetHoveredColumn() == columns_count)
        if (mouse_pos.y >= row_y1 and mouse_pos.y < row_y1 + row_height)
...
void ImGui::TableHeader(const char* label)
{
    ...
    ImGuiTable* table = g.CurrentTable;
    IM_ASSERT(table != NULL and "Need to call TableHeader() after BeginTable()!");
    ...
    if ((table->Flags & ImGuiTableFlags_Sortable) and !(column->Flags & ImGuiTableColumnFlags_NoSort))
    {
    ...
    if (held or hovered or highlight)
    {
    ...
    if (held and (table->Flags & ImGuiTableFlags_Reorderable) and IsMouseDragging(0) and !g.DragDropActive)
    {
        ...
        if (g.IO.MouseDelta.x < 0.0f and g.IO.MousePos.x < cell_r.Min.x)
            if (ImGuiTableColumn* prev_column = (column->PrevEnabledColumn != -1) ? &table->Columns[column->PrevEnabledColumn] : NULL)
                if (!((column->Flags | prev_column->Flags) & ImGuiTableColumnFlags_NoReorder))
                    if ((column->IndexWithinEnabledSet < table->FreezeColumnsRequest) == (prev_column->IndexWithinEnabledSet < table->FreezeColumnsRequest))
                        table->ReorderColumnDir = -1;
        if (g.IO.MouseDelta.x > 0.0f and g.IO.MousePos.x > cell_r.Max.x)
    ...
    if ((table->Flags & ImGuiTableFlags_Sortable) and !(column->Flags & ImGuiTableColumnFlags_NoSort))
    {
        ...
        if (pressed and table->ReorderColumn != column_n)
        {
    ...
    if (text_clipped and hovered and g.ActiveId == 0)
        SetItemTooltip("%.*s", (int)(label_end - label), label);

    // We don't use BeginPopupContextItem() because we want the popup to stay up even after the column is hidden
    if (IsMouseReleased(1) and IsItemHovered())
...
void ImGui::TableAngledHeadersRowEx(float angle, float max_label_width)
{
    ...
    IM_ASSERT(table != NULL and "Need to call TableHeadersRow() after BeginTable()!");
    IM_ASSERT(table->CurrentRow == -1 and "Must be first row");
    ...
    if (highlight_column_n == -1 and table->HoveredColumnBody != -1)
        if (table_instance->HoveredRowLast == 0 and table->HoveredColumnBorder == -1 and (g.ActiveId == 0 or g.ActiveId == row_id or (table->IsActiveIdInTable or g.DragDropActive)))
...
void ImGui::TableOpenContextMenu(int column_n)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    if (column_n == -1 and table->CurrentColumn != -1)   // When called within a column automatically use this one (for consistency)
        column_n = table->CurrentColumn;
    if (column_n == table->ColumnsCount)                // To facilitate using with TableGetHoveredColumn()
        column_n = -1;
    IM_ASSERT(column_n >= -1 and column_n < table->ColumnsCount);
...
{
    if (!table->IsContextPopupOpen or table->InstanceCurrent != table->InstanceInteracted)
        return false;
    const ImGuiID context_menu_id = ImHashStr("##ContextMenu", 0, table->ID);
...
void ImGui::TableDrawDefaultContextMenu(ImGuiTable* table, ImGuiTableFlags flags_for_section_to_display)
{
    ...
    const int column_n = (table->ContextPopupColumn >= 0 and table->ContextPopupColumn < table->ColumnsCount) ? table->ContextPopupColumn : -1;
    ...
    if (flags_for_section_to_display & ImGuiTableFlags_Resizable)
    {
        if (column != NULL)
        {
            const bool can_resize = !(column->Flags & ImGuiTableColumnFlags_NoResize) and column->IsEnabled;
            if (MenuItem(LocalizeGetMsg(ImGuiLocKey_TableSizeOne), NULL, false, can_resize)) // "###SizeOne"
                TableSetColumnWidthAutoSingle(table, column_n);
        }

        const char* size_all_desc;
        if (table->ColumnsEnabledFixedCount == table->ColumnsEnabledCount and (table->Flags & ImGuiTableFlags_SizingMask_) != ImGuiTableFlags_SizingFixedSame)
...
#if 0
    if ((flags_for_section_to_display & ImGuiTableFlags_Sortable) and column != NULL and (column->Flags & ImGuiTableColumnFlags_NoSort) == 0)
    {
        if (want_separator)
            Separator();
        want_separator = true;
        ...
        if (MenuItem("Sort in Ascending Order", NULL, column->SortOrder != -1 and column->SortDirection == ImGuiSortDirection_Ascending, (column->Flags & ImGuiTableColumnFlags_NoSortAscending) == 0))
            TableSetColumnSortDirection(table, column_n, ImGuiSortDirection_Ascending, append_to_sort_specs);
        if (MenuItem("Sort in Descending Order", NULL, column->SortOrder != -1 and column->SortDirection == ImGuiSortDirection_Descending, (column->Flags & ImGuiTableColumnFlags_NoSortDescending) == 0))
    ...
    if (flags_for_section_to_display & ImGuiTableFlags_Hideable)
    {
        ...
        for (int other_column_n = 0; other_column_n < table->ColumnsCount; other_column_n++)
        {
            ...
            if (other_column->IsUserEnabled and table->ColumnsEnabledCount <= 1)
...
void ImGui::TableSaveSettings(ImGuiTable* table)
{
    ...
    IM_ASSERT(settings->ID == table->ID);
    IM_ASSERT(settings->ColumnsCount == table->ColumnsCount and settings->ColumnsCountMax >= settings->ColumnsCount);
...
void ImGui::TableLoadSettings(ImGuiTable* table)
{
    ...
    for (int data_n = 0; data_n < settings->ColumnsCount; data_n++, column_settings++)
    {
        ...
        if (column_n < 0 or column_n >= table->ColumnsCount)
            continue;
...
static void TableSettingsHandler_ReadLine(ImGuiContext*, ImGuiSettingsHandler*, void* entry, const char* line)
{
    ...
    {
        if (column_n < 0 or column_n >= settings->ColumnsCount)
            return;
...
static void TableSettingsHandler_WriteAll(ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
    ...
    for (ImGuiTableSettings* settings = g.SettingsTables.begin(); settings != NULL; settings = g.SettingsTables.next_chunk(settings))
    {
        ...
        if (!save_size and !save_visible and !save_order and !save_sort)
            continue;
        ...
        {
            // "Column 0  UserID=0x42AD2D21 Width=100 Visible=1 Order=0 Sort=0v"
            bool save_column = column->UserID != 0 or save_size or save_visible or save_order or (save_sort and column->SortOrder != -1);
            if (!save_column)
                continue;
            buf->appendf("Column %-2d", column_n);
            if (column->UserID != 0)                    { buf->appendf(" UserID=%08X", column->UserID); }
            if (save_size and column->IsStretch)         { buf->appendf(" Weight=%.4f", column->WidthOrWeight); }
            if (save_size and !column->IsStretch)        { buf->appendf(" Width=%d", (int)column->WidthOrWeight); }
            if (save_visible)                           { buf->appendf(" Visible=%d", column->IsEnabled); }
            if (save_order)                             { buf->appendf(" Order=%d", column->DisplayOrder); }
            if (save_sort and column->SortOrder != -1)   { buf->appendf(" Sort=%d%c", column->SortOrder, (column->SortDirection == ImGuiSortDirection_Ascending) ? 'v' : '^'); }
            buf->append("\n");
        }
        buf->append("\n");
    }
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    if (IsItemVisible() and table->HoveredColumnBody != -1)
...
void ImGui::SetColumnOffset(int column_index, float offset)
{
    ...
    const bool preserve_width = !(columns->Flags & ImGuiOldColumnFlags_NoPreserveWidths) and (column_index < columns->Count - 1);
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags)
{
    ...
    if (columns->Columns.Size != 0 and columns->Columns.Size != columns_count + 1)
...
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems or window->DC.CurrentColumns == NULL)
        return;
...
void ImGui::EndColumns()
{
    ...
    if (!(flags & ImGuiOldColumnFlags_NoBorder) and !window->SkipItems)
    {
        ...
        for (int n = 1; n < columns->Count; n++)
        {
            ...
            if (!(flags & ImGuiOldColumnFlags_NoResize))
            {
                ...
                if (held and !(column->Flags & ImGuiOldColumnFlags_NoResize))
                    dragging_column = n;
            }
...
void ImGui::Columns(int columns_count, const char* id, bool border)
{
    ...
    ImGuiOldColumns* columns = window->DC.CurrentColumns;
    if (columns != NULL and columns->Count == columns_count and columns->Flags == flags)
        return;
