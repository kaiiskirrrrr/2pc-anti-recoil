...
#if (defined __SSE__ or defined __x86_64__ or defined _M_X64 or (defined(_M_IX86_FP) and (_M_IX86_FP >= 1))) and !defined(ImGui_DISABLE_SSE)
...
#if defined(ImGui_DEFINE_MATH_OPERATORS) and !defined(ImGui_DEFINE_MATH_OPERATORS_IMPLEMENTED)
...
struct ImGuiContext
{
    ...
    ImGuiID                 NavActivateId;                      // ~~ (g.ActiveId == 0) and (IsKeyPressed(ImGuiKey_Space) or IsKeyDown(ImGuiKey_Enter) or IsKeyPressed(ImGuiKey_NavGamepadActivate)) ? NavId : 0, also set when calling ActivateItem()
    ImGuiID                 NavActivateDownId;                  // ~~ IsKeyDown(ImGuiKey_Space) or IsKeyDown(ImGuiKey_Enter) or IsKeyDown(ImGuiKey_NavGamepadActivate) ? NavId : 0
    ImGuiID                 NavActivatePressedId;               // ~~ IsKeyPressed(ImGuiKey_Space) or IsKeyPressed(ImGuiKey_Enter) or IsKeyPressed(ImGuiKey_NavGamepadActivate) ? NavId : 0 (no repeat)
...
struct ImGuiTableColumn
{
    ...
    bool                    IsEnabled;                      // IsUserEnabled and (Flags & ImGuiTableColumnFlags_Disabled) == 0
...
//#endif
    ...
    inline bool             IsNamedKey(ImGuiKey key)                                    { return key >= ImGuiKey_NamedKey_BEGIN and key < ImGuiKey_NamedKey_END; }
    inline bool             IsNamedKeyOrModKey(ImGuiKey key)                            { return (key >= ImGuiKey_NamedKey_BEGIN and key < ImGuiKey_NamedKey_END) or key == ImGuiMod_Ctrl or key == ImGuiMod_Shift or key == ImGuiMod_Alt or key == ImGuiMod_Super or key == ImGuiMod_Shortcut; }
    inline bool             IsLegacyKey(ImGuiKey key)                                   { return key >= ImGuiKey_LegacyNativeKey_BEGIN and key < ImGuiKey_LegacyNativeKey_END; }
    inline bool             IsKeyboardKey(ImGuiKey key)                                 { return key >= ImGuiKey_Keyboard_BEGIN and key < ImGuiKey_Keyboard_END; }
    inline bool             IsGamepadKey(ImGuiKey key)                                  { return key >= ImGuiKey_Gamepad_BEGIN and key < ImGuiKey_Gamepad_END; }
    inline bool             IsMouseKey(ImGuiKey key)                                    { return key >= ImGuiKey_Mouse_BEGIN and key < ImGuiKey_Mouse_END; }
    inline bool             IsAliasKey(ImGuiKey key)                                    { return key >= ImGuiKey_Aliases_BEGIN and key < ImGuiKey_Aliases_END; }
    inline bool             IsModKey(ImGuiKey key)                                      { return key >= ImGuiKey_LeftCtrl and key <= ImGuiKey_RightSuper; }
    ...
    inline ImGuiKey         MouseButtonToKey(ImGuiMouseButton button)                   { IM_ASSERT(button >= 0 and button < ImGuiMouseButton_COUNT); return (ImGuiKey)(ImGuiKey_MouseLeft + button); }
    ...
    ImGui_API void              SetItemKeyOwner(ImGuiKey key, ImGuiInputFlags flags = 0);           // Set key owner to last item if it is hovered or active. Equivalent to 'if (IsItemHovered() or IsItemActive()) { SetKeyOwner(key, GetItemID());'.
    ...
    ImGui_API bool          ColorButtonEx(const char* label, ImVec4 color, const ImVec2& size_arg = ImVec2(0, 0), ImGuiButtonFlags flags = 0);
    ...
    inline bool             TempInputIsActive(ImGuiID id)       { ImGuiContext& g = *GImGui; return (g.ActiveId == id and g.TempInputId == id); }
    inline ImGuiInputTextState* GetInputTextState(ImGuiID id)   { ImGuiContext& g = *GImGui; return (id != 0 and g.InputTextState.ID == id) ? &g.InputTextState : NULL; } // Get input text state if active
    ...
    //  (New) ImGui_VERSION_NUM >= 18413: using 'ItemAdd(..., ImGuiItemFlags_Inputable)'     and 'bool tab_focused = (g.NavActivateId == id and (g.NavActivateFlags & ImGuiActivateFlags_PreferInput))'
...
#endif
#ifndef ImGui_DISABLE_OBSOLETE_KEYIO
    inline bool     IsKeyPressedMap(ImGuiKey key, bool repeat = true)                   { IM_ASSERT(IsNamedKey(key)); return IsKeyPressed(key, repeat); } // Removed in 1.87: Mapping from named key is always idactor!
#endif
