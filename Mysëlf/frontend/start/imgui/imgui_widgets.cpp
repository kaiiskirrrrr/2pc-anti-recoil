...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    const bool flatten_hovered_children = (flags & ImGuiButtonFlags_FlattenChildren) and g.HoveredWindow and g.HoveredWindow->RootWindow == window;
    ...
    if (g.DragDropActive and (flags & ImGuiButtonFlags_PressedOnDragDropHold) and !(g.DragDropSourceFlags & ImGuiDragDropFlags_SourceNoHoldToOpenOthers))
    ...
    if (hovered)
    {
        ...
        if (!(flags & ImGuiButtonFlags_NoKeyModifiers) or (!g.IO.KeyCtrl and !g.IO.KeyShift and !g.IO.KeyAlt))
        {
            if (mouse_button_clicked != -1 and g.ActiveId != id)
            {
                ...
                if ((flags & ImGuiButtonFlags_PressedOnClick) or ((flags & ImGuiButtonFlags_PressedOnDoubleClick) and g.IO.MouseClickedCount[mouse_button_clicked] == 2))
                {
                    pressed = true;
            ...
            if (flags & ImGuiButtonFlags_PressedOnRelease)
            {
                ...
                {
                    const bool has_repeated_at_least_once = (item_flags & ImGuiItemFlags_ButtonRepeat) and g.IO.MouseDownDurationPrev[mouse_button_released] >= g.IO.KeyRepeatDelay; // Repeat mode trumps on release behavior
            ...
            if (g.ActiveId == id and (item_flags & ImGuiItemFlags_ButtonRepeat))
                if (g.IO.MouseDownDuration[g.ActiveIdMouseButton] > 0.0f and IsMouseClicked(g.ActiveIdMouseButton, test_owner_id, ImGuiInputFlags_Repeat))
                    pressed = true;
        }
    ...
    if (g.NavActivateDownId == id)
    {
        ...
        if (!nav_activated_by_inputs and (item_flags & ImGuiItemFlags_ButtonRepeat))
        {
        ...
        if (nav_activated_by_code or nav_activated_by_inputs)
        {
            ...
            if (!(flags & ImGuiButtonFlags_NoNavFocus) and !(g.NavActivateFlags & ImGuiActivateFlags_FromShortcut))
                SetFocusID(id, window);
    ...
    if (g.ActiveId == id)
    {
        if (g.ActiveIdSource == ImGuiInputSource_Mouse)
        {
            ...
            {
                bool release_in = hovered and (flags & ImGuiButtonFlags_PressedOnClickRelease) != 0;
                bool release_anywhere = (flags & ImGuiButtonFlags_PressedOnClickReleaseAnywhere) != 0;
                if ((release_in or release_anywhere) and !g.DragDropActive)
                {
                    // Report as pressed when releasing the mouse (this is the most common path)
                    bool is_double_click_release = (flags & ImGuiButtonFlags_PressedOnDoubleClick) and g.IO.MouseReleased[mouse_button] and g.IO.MouseClickedLastCount[mouse_button] == 2;
                    bool is_repeating_already = (item_flags & ImGuiItemFlags_ButtonRepeat) and g.IO.MouseDownDurationPrev[mouse_button] >= g.IO.KeyRepeatDelay; // Repeat mode trumps <on release>
        ...
        else if (g.ActiveIdSource == ImGuiInputSource_Keyboard or g.ActiveIdSource == ImGuiInputSource_Gamepad)
        {
...
bool ImGui::ColorButtonEx(const char* label, ImVec4 color, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec2 pos = window->DC.CursorPos;
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) and style.FramePadding.y < window->DC.CurrLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flag)
    ...
    // Render
    const ImU32 col = GetColorU32((held and hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    ...
    //if (pressed and !(flags & ImGuiButtonFlags_DontClosePopups) and (window->Flags & ImGuiWindowFlags_Popup))
    //    CloseCurrentPopup();
...
bool ImGui::ColorButton1(const char* label, ImVec4 color,  const ImVec2& size_arg)
{
    return ColorButtonEx(label, color, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::ArrowButtonEx(const char* str_id, ImGuiDir dir, ImVec2 size, ImGuiButtonFlags flags)
{
    ...
    // Render
    const ImU32 bg_col = GetColorU32((held and hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    // Render
    ImU32 bg_col = GetColorU32((held and hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    ImU32 text_col = GetColorU32(ImGuiCol_Text);
    if (hovered or held)
...
void ImGui::Scrollbar(ImGuiAxis axis)
{
    ...
    {
        if ((window->Flags & ImGuiWindowFlags_NoTitleBar) and !(window->Flags & ImGuiWindowFlags_MenuBar))
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    float alpha = 1.0f;
    if ((axis == ImGuiAxis_Y) and bb_frame_height < g.FontSize + g.Style.FramePadding.y * 2.0f)
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((held and hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, style.FrameRounding);
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    window->DrawList->AddCircleFilled(center, radius, GetColorU32((held and hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), num_segment);
    if (active)
    {
        const float pad = ImMax(1.0f, IM_TRUNC(square_sz / 6.0f));
...
bool ImGui::SplitterBehavior(const ImRect& bb, ImGuiID id, ImGuiAxis axis, float* size1, float* size2, float min_size1, float min_size2, float hover_extend, float hover_visibility_delay, ImU32 bg_col)
{
    ...
    const ImU32 col = GetColorU32(held ? ImGuiCol_SeparatorActive : (hovered and g.HoveredIdTimer >= hover_visibility_delay) ? ImGuiCol_SeparatorHovered : ImGuiCol_Separator);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float preview_width = ((flags & ImGuiComboFlags_WidthFitPreview) and (preview_value != NULL)) ? CalcTextSize(preview_value, NULL, true).x : 0.0f;
    ...
    {
        ImU32 bg_col = GetColorU32((popup_open or hovered) ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
    ...
    if (preview_value != NULL and !(flags & ImGuiComboFlags_NoPreview))
    {
        if (g.LogEnabled)
            LogSetNextTextDecoration("{", "}");
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    else
    {
        ...
        if ((g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSize) == 0 or g.NextWindowData.SizeVal.x <= 0.0f) // Don't apply constraints if user specified a size
            constraint_min.x = w;
        if ((g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSize) == 0 or g.NextWindowData.SizeVal.y <= 0.0f)
...
bool ImGui::BeginComboPreview()
{
    ...
    if (window->SkipItems or !(g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible))
        return false;
...
bool ImGui::Combo(const char* label, int* current_item, const char* (*getter)(void* user_data, int idx), void* user_data, int items_count, int popup_max_height_in_items)
{
    ...
    if (popup_max_height_in_items != -1 and !(g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSizeConstraint))
...
{
    IM_ASSERT(data_type >= 0 and data_type < ImGuiDataType_COUNT);
...
int ImGui::DataTypeFormatString(char* buf, int buf_size, ImGuiDataType data_type, const void* p_data, const char* format)
{
    ...
    if (data_type == ImGuiDataType_S32 or data_type == ImGuiDataType_U32)
        return ImFormatString(buf, buf_size, format, *(const ImU32*)p_data);
    if (data_type == ImGuiDataType_S64 or data_type == ImGuiDataType_U64)
...
bool ImGui::DataTypeApplyFromText(const char* buf, ImGuiDataType data_type, void* p_data, const char* format)
{
    ...
    if (data_type == ImGuiDataType_Float or data_type == ImGuiDataType_Double)
...
{
    IM_UNUSED(data_type);
    IM_ASSERT(data_type == ImGuiDataType_Float or data_type == ImGuiDataType_Double);
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const bool is_floating_point = (data_type == ImGuiDataType_Float) or (data_type == ImGuiDataType_Double);
    ...
    if (g.ActiveIdSource == ImGuiInputSource_Mouse and IsMousePosValid() and IsMouseDragPastThreshold(0, g.IO.MouseDragThreshold * DRAG_MOUSE_THRESHOLD_FACTOR))
    {
    ...
    if (is_floating_point and !(flags & ImGuiSliderFlags_NoRoundToFormat))
...
bool ImGui::DragBehavior(ImGuiID id, ImGuiDataType data_type, void* p_v, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    IM_ASSERT((flags == 1 or (flags & ImGuiSliderFlags_InvalidMask_) == 0) and "Invalid ImGuiSliderFlags flags! Has the 'float power' argument been mistakenly cast to flags? Call function with ImGuiSliderFlags_Logarithmic flags instead.");

    ImGuiContext& g = *GImGui;
    if (g.ActiveId == id)
    {
        // Those are the things we can do easily outside the DragBehaviorT<> template, saves code generation.
        if (g.ActiveIdSource == ImGuiInputSource_Mouse and !g.IO.MouseDown[0])
            ClearActiveID();
        else if ((g.ActiveIdSource == ImGuiInputSource_Keyboard or g.ActiveIdSource == ImGuiInputSource_Gamepad) and g.NavActivatePressedId == id and !g.ActiveIdIsJustActivated)
            ClearActiveID();
    }
    if (g.ActiveId != id)
        return false;
    if ((g.LastItemData.InFlags & ImGuiItemFlags_ReadOnly) or (flags & ImGuiSliderFlags_ReadOnly))
        return false;
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (!temp_input_is_active)
    {
        ...
        const bool double_clicked = (hovered and g.IO.MouseClickedCount[0] == 2 and TestKeyOwner(ImGuiKey_MouseLeft, id));
    ...
    if (temp_input_is_active)
    {
        ...
        const bool is_clamp_input = (flags & ImGuiSliderFlags_AlwaysClamp) != 0 and (p_min == NULL or p_max == NULL or DataTypeCompare(data_type, p_min, p_max) < 0);
...
{
    // Read ImGui.cpp "API BREAKING CHANGES" section for 1.78 if you hit this assert.
    IM_ASSERT((flags == 1 or (flags & ImGuiSliderFlags_InvalidMask_) == 0) and "Invalid ImGuiSliderFlags flag!  Has the 'float power' argument been mistakenly cast to flags? Call function with ImGuiSliderFlags_Logarithmic flags instead.");
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (!temp_input_is_active)
    {
        ...
        if (make_active and clicked)
            SetKeyOwner(ImGuiKey_MouseLeft, id);
        if (make_active and temp_input_allowed)
            if ((clicked and g.IO.KeyCtrl) or (g.NavActivateId == id and (g.NavActivateFlags & ImGuiActivateFlags_PreferInput)))
                temp_input_is_active = true;
...
static int is_word_boundary_from_right(ImGuiInputTextState* obj, int idx)
{
    ...
    if ((obj->Flags & ImGuiInputTextFlags_Password) or idx <= 0)
        return 0;
...
static int  STB_TEXTEDIT_MOVEWORDLEFT_IMPL(ImGuiInputTextState* obj, int idx)   { idx--; while (idx >= 0 and !is_word_boundary_from_right(obj, idx)) idx--; return idx < 0 ? 0 : idx; }
static int  STB_TEXTEDIT_MOVEWORDRIGHT_MAC(ImGuiInputTextState* obj, int idx)   { idx++; int len = obj->CurLenW; while (idx < len and !is_word_boundary_from_left(obj, idx)) idx++; return idx > len ? len : idx; }
static int  STB_TEXTEDIT_MOVEWORDRIGHT_WIN(ImGuiInputTextState* obj, int idx)   { idx++; int len = obj->CurLenW; while (idx < len and !is_word_boundary_from_right(obj, idx)) idx++; return idx > len ? len : idx; }
...
static bool InputTextFilterCharacter(ImGuiContext* ctx, unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, bool input_source_is_clipboard)
{
    ...
    {
        bool pass = false;
        pass |= (c == '\n') and (flags & ImGuiInputTextFlags_Multiline) != 0; // Note that an Enter KEY will emit \r and be ignored (we poll for KEY in InputText() code)
        pass |= (c == '\t') and (flags & ImGuiInputTextFlags_AllowTabInput) != 0;
        if (!pass)
            return false;
    ...
    if (apply_named_filters and (flags & (ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase | ImGuiInputTextFlags_CharsNoBlank | ImGuiInputTextFlags_CharsScientific | (ImGuiInputTextFlags)ImGuiInputTextFlags_LocalizeDecimalPoint)))
    {
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    IM_ASSERT(!((flags & ImGuiInputTextFlags_CallbackHistory) and (flags & ImGuiInputTextFlags_Multiline)));        // Can't use both together (they both use up/down keys)
    IM_ASSERT(!((flags & ImGuiInputTextFlags_CallbackCompletion) and (flags & ImGuiInputTextFlags_AllowTabInput))); // Can't use both together (they both use tab key)
    ...
    if (is_multiline)
    {
        ...
        if (g.NavActivateId == id and (g.NavActivateFlags & ImGuiActivateFlags_FromTabbing) and (flags & ImGuiInputTextFlags_AllowTabInput))
            g.NavActivateId = 0;
    ...
    const bool input_requested_by_nav = (g.ActiveId != id) and ((g.NavActivateId == id) and ((g.NavActivateFlags & ImGuiActivateFlags_PreferInput) or (g.NavInputSource == ImGuiInputSource_Keyboard)));

    const bool user_clicked = hovered and io.MouseClicked[0];
    const bool user_scroll_finish = is_multiline and state != NULL and g.ActiveId == 0 and g.ActiveIdPreviousFrame == GetWindowScrollbarID(draw_window, ImGuiAxis_Y);
    const bool user_scroll_active = is_multiline and state != NULL and g.ActiveId == GetWindowScrollbarID(draw_window, ImGuiAxis_Y);
    ...
    if ((init_state and g.ActiveId != id) or init_changed_specs or init_reload_from_user_buf)
    {
        ...
        else if (!is_multiline)
        {
            if (flags & ImGuiInputTextFlags_AutoSelectAll)
                select_all = true;
            if (input_requested_by_nav and (!recycle_state or !(g.NavActivateFlags & ImGuiActivateFlags_TryToPreserveState)))
                select_all = true;
    ...
    if (g.ActiveId == id)
    {
        ...
        if (user_clicked)
            SetKeyOwner(ImGuiKey_MouseLeft, id);
        g.ActiveIdUsingNavDirMask |= (1 << ImGuiDir_Left) | (1 << ImGuiDir_Right);
        if (is_multiline or (flags & ImGuiInputTextFlags_CallbackHistory))
            g.ActiveIdUsingNavDirMask |= (1 << ImGuiDir_Up) | (1 << ImGuiDir_Down);
    ...
    if (g.ActiveId == id)
    {
        ...
        if ((flags & ImGuiInputTextFlags_AllowTabInput) and !is_readonly)
        {
    ...
    if (g.ActiveId == id and !g.ActiveIdIsJustActivated and !clear_active_id)
    {
        ...
        const bool is_cut   = (Shortcut(ImGuiMod_Shortcut | ImGuiKey_X, id, f_repeat) or Shortcut(ImGuiMod_Shift | ImGuiKey_Delete, id, f_repeat)) and !is_readonly and !is_password and (!is_multiline or state->HasSelection());
        const bool is_copy  = (Shortcut(ImGuiMod_Shortcut | ImGuiKey_C, id) or Shortcut(ImGuiMod_Ctrl | ImGuiKey_Insert, id))  and !is_password and (!is_multiline or state->HasSelection());
        const bool is_paste = (Shortcut(ImGuiMod_Shortcut | ImGuiKey_V, id, f_repeat) or Shortcut(ImGuiMod_Shift | ImGuiKey_Insert, id, f_repeat)) and !is_readonly;
        const bool is_undo  = (Shortcut(ImGuiMod_Shortcut | ImGuiKey_Z, id, f_repeat)) and !is_readonly and is_undoable;
        const bool is_redo =  (Shortcut(ImGuiMod_Shortcut | ImGuiKey_Y, id, f_repeat) or (is_osx and Shortcut(ImGuiMod_Shortcut | ImGuiMod_Shift | ImGuiKey_Z, id, f_repeat))) and !is_readonly and is_undoable;
        ...
        const bool is_enter_pressed = IsKeyPressed(ImGuiKey_Enter, true) or IsKeyPressed(ImGuiKey_KeypadEnter, true);
        const bool is_gamepad_validate = nav_gamepad_active and (IsKeyPressed(ImGuiKey_NavGamepadActivate, false) or IsKeyPressed(ImGuiKey_NavGamepadInput, false));
        const bool is_cancel = Shortcut(ImGuiKey_Escape, id, f_repeat) or (nav_gamepad_active and Shortcut(ImGuiKey_NavGamepadCancel, id, f_repeat));

        // FIXME: Should use more Shortcut() and reduce IsKeyPressed()+SetKeyOwner(), but requires modifiers combination to be taken account of.
        if (IsKeyPressed(ImGuiKey_LeftArrow))                        { state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_LINESTART : is_wordmove_key_down ? STB_TEXTEDIT_K_WORDLEFT : STB_TEXTEDIT_K_LEFT) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_RightArrow))                  { state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_LINEEND : is_wordmove_key_down ? STB_TEXTEDIT_K_WORDRIGHT : STB_TEXTEDIT_K_RIGHT) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_UpArrow) and is_multiline)     { if (io.KeyCtrl) SetScrollY(draw_window, ImMax(draw_window->Scroll.y - g.FontSize, 0.0f)); else state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTSTART : STB_TEXTEDIT_K_UP) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_DownArrow) and is_multiline)   { if (io.KeyCtrl) SetScrollY(draw_window, ImMin(draw_window->Scroll.y + g.FontSize, GetScrollMaxY())); else state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTEND : STB_TEXTEDIT_K_DOWN) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_PageUp) and is_multiline)      { state->OnKeyPressed(STB_TEXTEDIT_K_PGUP | k_mask); scroll_y -= row_count_per_page * g.FontSize; }
        else if (IsKeyPressed(ImGuiKey_PageDown) and is_multiline)    { state->OnKeyPressed(STB_TEXTEDIT_K_PGDOWN | k_mask); scroll_y += row_count_per_page * g.FontSize; }
        else if (IsKeyPressed(ImGuiKey_Home))                        { state->OnKeyPressed(io.KeyCtrl ? STB_TEXTEDIT_K_TEXTSTART | k_mask : STB_TEXTEDIT_K_LINESTART | k_mask); }
        else if (IsKeyPressed(ImGuiKey_End))                         { state->OnKeyPressed(io.KeyCtrl ? STB_TEXTEDIT_K_TEXTEND | k_mask : STB_TEXTEDIT_K_LINEEND | k_mask); }
        else if (IsKeyPressed(ImGuiKey_Delete) and !is_readonly and !is_cut)
        {
        ...
        else if (IsKeyPressed(ImGuiKey_Backspace) and !is_readonly)
        {
            if (!state->HasSelection())
            {
                if (is_wordmove_key_down)
    ...
    if (g.ActiveId == id)
    {
        ...
        const bool apply_edit_back_to_user_buffer = !revert_edit or (validated and (flags & ImGuiInputTextFlags_EnterReturnsTrue) != 0);
        if (apply_edit_back_to_user_buffer)
        {
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackEdit | ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                if ((flags & ImGuiInputTextFlags_CallbackCompletion) != 0 and Shortcut(ImGuiKey_Tab, id))
                {
                    event_flag = ImGuiInputTextFlags_CallbackCompletion;
                    event_key = ImGuiKey_Tab;
                }
                else if ((flags & ImGuiInputTextFlags_CallbackHistory) != 0 and IsKeyPressed(ImGuiKey_UpArrow))
                {
                    event_flag = ImGuiInputTextFlags_CallbackHistory;
                    event_key = ImGuiKey_UpArrow;
                }
                else if ((flags & ImGuiInputTextFlags_CallbackHistory) != 0 and IsKeyPressed(ImGuiKey_DownArrow))
                {
                    event_flag = ImGuiInputTextFlags_CallbackHistory;
                    event_key = ImGuiKey_DownArrow;
                }
                else if ((flags & ImGuiInputTextFlags_CallbackEdit) and state->Edited)
                {
    ...
    if (value_changed and !(flags & ImGuiInputTextFlags_NoMarkEdited))
        MarkItemEdited(id);
...
static void ColorEditRestoreH(const float* col, float* H)
{
    ...
    if (g.ColorEditSavedID != g.ColorEditCurrentID or g.ColorEditSavedColor != ImGui::ColorConvertFloat4ToU32(ImVec4(col[0], col[1], col[2], 0)))
        return;
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if ((flags & ImGuiColorEditFlags_InputHSV) and (flags & ImGuiColorEditFlags_DisplayRGB))
        ColorConvertHSVtoRGB(f[0], f[1], f[2], f[0], f[1], f[2]);
    else if ((flags & ImGuiColorEditFlags_InputRGB) and (flags & ImGuiColorEditFlags_DisplayHSV))
    {
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 and (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
    ...
    else if ((flags & ImGuiColorEditFlags_DisplayHex) != 0 and (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
    ...
    {
        const float button_offset_x = ((flags & ImGuiColorEditFlags_NoInputs) or (style.ColorButtonPosition == ImGuiDir_Left)) ? 0.0f : w_inputs + style.ItemInnerSpacing.x;
    ...
    if (label != label_display_end and !(flags & ImGuiColorEditFlags_NoLabel))
    {
    ...
    if (value_changed and picker_active_window == NULL)
    {
        if (!value_changed_as_float)
            for (int n = 0; n < 4; n++)
                f[n] = i[n] / 255.0f;
        if ((flags & ImGuiColorEditFlags_DisplayHSV) and (flags & ImGuiColorEditFlags_InputRGB))
        {
        ...
        if ((flags & ImGuiColorEditFlags_DisplayRGB) and (flags & ImGuiColorEditFlags_InputHSV))
            ColorConvertRGBtoHSV(f[0], f[1], f[2], f[0], f[1], f[2]);
    ...
    if ((g.LastItemData.StatusFlags & ImGuiItemStatusFlags_HoveredRect) and !(g.LastItemData.InFlags & ImGuiItemFlags_ReadOnly) and !(flags & ImGuiColorEditFlags_NoDragDrop) and BeginDragDropTarget())
    {
        ...
        if (accepted_drag_drop and (flags & ImGuiColorEditFlags_InputHSV))
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    bool alpha_bar = (flags & ImGuiColorEditFlags_AlphaBar) and !(flags & ImGuiColorEditFlags_NoAlpha);
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (flags & ImGuiColorEditFlags_DisplayRGB or (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
        ...
        if (flags & ImGuiColorEditFlags_DisplayHSV or (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            value_changed |= ColorEdit4("##hsv", col, sub_flags | ImGuiColorEditFlags_DisplayHSV);
        if (flags & ImGuiColorEditFlags_DisplayHex or (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            value_changed |= ColorEdit4("##hex", col, sub_flags | ImGuiColorEditFlags_DisplayHex);
        PopItemWidth();
    }

    // Try to cancel hue wrap (after ColorEdit4 call), if any
    if (value_changed_fix_hue_wrap and (flags & ImGuiColorEditFlags_InputRGB))
    {
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) and col_rgb.w < 1.0f)
    {
    ...
    if (g.ActiveId == id and !(flags & ImGuiColorEditFlags_NoDragDrop) and BeginDragDropSource())
    {
        if (flags & ImGuiColorEditFlags_NoAlpha)
    ...
    // Tooltip
    if (!(flags & ImGuiColorEditFlags_NoTooltip) and hovered and IsItemHovered(ImGuiHoveredFlags_ForTooltip))
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    SameLine();
    if ((flags & ImGuiColorEditFlags_InputRGB) or !(flags & ImGuiColorEditFlags_InputMask_))
    {
        if (flags & ImGuiColorEditFlags_NoAlpha)
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    bool allow_opt_alpha_bar = !(flags & ImGuiColorEditFlags_NoAlpha) and !(flags & ImGuiColorEditFlags_AlphaBar);
...
bool ImGui::TreeNodeUpdateNextOpen(ImGuiID id, ImGuiTreeNodeFlags flags)
{
    ...
    if (g.LogEnabled and !(flags & ImGuiTreeNodeFlags_NoAutoOpenOnLog) and (window->DC.TreeDepth - g.LogDepthRef) < g.LogDepthToExpand)
        is_open = true;
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    const bool span_all_columns = (flags & ImGuiTreeNodeFlags_SpanAllColumns) != 0 and (g.CurrentTable != NULL);
    ImRect frame_bb;
    ...
    if (!display_frame and (flags & (ImGuiTreeNodeFlags_SpanAvailWidth | ImGuiTreeNodeFlags_SpanFullWidth | ImGuiTreeNodeFlags_SpanAllColumns)) == 0)
    ...
    if (is_open and !g.NavIdIsAlive and (flags & ImGuiTreeNodeFlags_NavLeftJumpsBackHere) and !(flags & ImGuiTreeNodeFlags_NoTreePushOnOpen))
        if (g.NavMoveDir == ImGuiDir_Left and g.NavWindow == window and NavMoveRequestButNoResultYet())
        {
    ...
    if ((flags & ImGuiTreeNodeFlags_AllowOverlap) or (g.LastItemData.InFlags & ImGuiItemFlags_AllowOverlap))
        button_flags |= ImGuiButtonFlags_AllowOverlap;
    if (!is_leaf)
    ...
    if (!is_leaf)
    {
        if (pressed and g.DragDropHoldJustPressedId != id)
        {
            ...
            if ((flags & ImGuiTreeNodeFlags_OpenOnDoubleClick) and g.IO.MouseClickedCount[0] == 2)
                toggled = true;
        }
    ...
    {
        // Framed type
        const ImU32 bg_col = GetColorU32((held and hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
...
void ImGui::TreePop()
{
    ...
    if (window->DC.TreeJumpToParentOnPopMask & tree_depth_mask) // Only set during request
    {
        ...
        if (g.NavIdIsAlive and g.NavMoveDir == ImGuiDir_Left and g.NavWindow == window and NavMoveRequestButNoResultYet())
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (size_arg.x == 0.0f or (flags & ImGuiSelectableFlags_SpanAvailWidth))
    ...
    if ((flags & ImGuiSelectableFlags_AllowOverlap) or (g.LastItemData.InFlags & ImGuiItemFlags_AllowOverlap)) { button_flags |= ImGuiButtonFlags_AllowOverlap; }
    ...
    if (pressed or (hovered and (flags & ImGuiSelectableFlags_SetNavIdOnHover)))
    {
    ...
    if (hovered or selected)
    {
        const ImU32 col = GetColorU32((held and hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
    ...
    // Automatically close popups
    if (pressed and (window->Flags & ImGuiWindowFlags_Popup) and !(flags & ImGuiSelectableFlags_DontClosePopups) and !(g.LastItemData.InFlags & ImGuiItemFlags_SelectableDontClosePopup))
        CloseCurrentPopup();
...
ImGuiTypingSelectRequest* ImGui::GetTypingSelectRequest(ImGuiTypingSelectFlags flags)
{
    ...
    if (data->SearchBuffer[0] != 0)
    {
        ...
        clear_buffer |= IsKeyPressed(ImGuiKey_Escape) or IsKeyPressed(ImGuiKey_Enter);
        clear_buffer |= IsKeyPressed(ImGuiKey_Backspace) and (flags & ImGuiTypingSelectFlags_AllowBackspace) == 0;
    ...
    // Handle backspace
    if ((flags & ImGuiTypingSelectFlags_AllowBackspace) and IsKeyPressed(ImGuiKey_Backspace, 0, ImGuiInputFlags_Repeat))
    {
...
int ImGui::TypingSelectFindMatch(ImGuiTypingSelectRequest* req, int items_count, const char* (*get_item_name_func)(void*, int), void* user_data, int nav_item_idx)
{
    if (req == NULL or req->SelectRequest == false) // Support NULL parameter so both calls can be done from same spot.
        return -1;
    int idx = -1;
    if (req->SingleCharMode and (req->Flags & ImGuiTypingSelectFlags_AllowSingleCharMode))
...
void ImGui::EndListBox()
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    IM_ASSERT((window->Flags & ImGuiWindowFlags_ChildWindow) and "Mismatched BeginListBox/EndListBox calls. Did you test the return value of BeginListBox?");
    IM_UNUSED(window);
...
void ImGui::EndMenuBar()
{
    ...
    if (NavMoveRequestButNoResultYet() and (g.NavMoveDir == ImGuiDir_Left or g.NavMoveDir == ImGuiDir_Right) and (g.NavWindow->Flags & ImGuiWindowFlags_ChildMenu))
    {
        // Try to find out if the request is for one of our child menu
        ImGuiWindow* nav_earliest_child = g.NavWindow;
        while (nav_earliest_child->ParentWindow and (nav_earliest_child->ParentWindow->Flags & ImGuiWindowFlags_ChildMenu))
            nav_earliest_child = nav_earliest_child->ParentWindow;
        if (nav_earliest_child->ParentWindow == window and nav_earliest_child->DC.ParentLayoutType == ImGuiLayoutType_Horizontal and (g.NavMoveFlags & ImGuiNavMoveFlags_Forwarded) == 0)
        {
...
bool ImGui::BeginViewportSideBar(const char* name, ImGuiViewport* viewport_p, ImGuiDir dir, float axis_size, ImGuiWindowFlags window_flags)
{
    ...
    if (bar_window == NULL or bar_window->BeginCount == 0)
    {
        ...
        ImGuiAxis axis = (dir == ImGuiDir_Up or dir == ImGuiDir_Down) ? ImGuiAxis_Y : ImGuiAxis_X;
        ImVec2 pos = avail_rect.Min;
        if (dir == ImGuiDir_Right or dir == ImGuiDir_Down)
        ...
        if (dir == ImGuiDir_Up or dir == ImGuiDir_Left)
            viewport->BuildWorkOffsetMin[axis] += axis_size;
        else if (dir == ImGuiDir_Down or dir == ImGuiDir_Right)
...
void ImGui::EndMainMenuBar()
{
    ...
    ImGuiContext& g = *GImGui;
    if (g.CurrentWindow == g.NavWindow and g.NavLayer == ImGuiNavLayer_Main and !g.NavAnyRequest)
...
static bool IsRootOfOpenMenuSet()
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if ((g.OpenPopupStack.Size <= g.BeginPopupStack.Size) or (window->Flags & ImGuiWindowFlags_ChildMenu))
        return false;
    ...
    if (window->DC.NavLayerCurrent != upper_popup->ParentNavLayer)
        return false;
    return upper_popup->Window and (upper_popup->Window->Flags & ImGuiWindowFlags_ChildMenu) and ImGui::IsWindowChildOf(upper_popup->Window, window, true);
}
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        ImGuiWindow* child_menu_window = (child_popup and child_popup->Window and child_popup->Window->ParentWindow == window) ? child_popup->Window : NULL;
...
void ImGui::EndMenu()
{
    ...
    if (window->BeginCount == window->BeginCountPreviousFrame)
        if (g.NavMoveDir == ImGuiDir_Left and NavMoveRequestButNoResultYet())
            if (g.NavWindow and (g.NavWindow->RootWindowForNav == window) and parent_window->DC.LayoutType == ImGuiLayoutType_Vertical)
            {
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
    ...
    if ((flags & ImGuiTabBarFlags_Reorderable) != (tab_bar->Flags & ImGuiTabBarFlags_Reorderable) or (tab_bar->TabsAddedNew and !(flags & ImGuiTabBarFlags_Reorderable)))
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        if ((most_recently_selected_tab == NULL or most_recently_selected_tab->LastFrameSelected < tab->LastFrameSelected) and !(tab->Flags & ImGuiTabItemFlags_Button))
        ...
        const bool has_close_button_or_unsaved_marker = (tab->Flags & ImGuiTabItemFlags_NoCloseButton) == 0 or (tab->Flags & ImGuiTabItemFlags_UnsavedDocument);
    ...
    if ((tab_bar->WidthAllTabsIdeal > tab_bar->BarRect.GetWidth() and tab_bar->Tabs.Size > 1) and !(tab_bar->Flags & ImGuiTabBarFlags_NoTabListScrollingButtons) and (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyScroll))
    ...
    if (width_excess >= 1.0f and ((tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyResizeDown) or !central_section_is_visible))
    {
    ...
    else if ((tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyScroll) and IsMouseHoveringRect(tab_bar->BarRect.Min, tab_bar->BarRect.Max, true) and IsWindowContentHoverable(g.CurrentWindow))
    {
...
bool ImGui::TabBarProcessReorder(ImGuiTabBar* tab_bar)
{
    ...
    if (tab1 == NULL or (tab1->Flags & ImGuiTabItemFlags_NoReorder))
        return false;
...
bool    ImGui::BeginTabItem(const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    if (ret and !(flags & ImGuiTabItemFlags_NoPushId))
    {
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    IM_ASSERT(!p_open or !(flags & ImGuiTabItemFlags_Button));
    ...
    ImVec2 size = TabItemCalcSize(label, (p_open != NULL) or (flags & ImGuiTabItemFlags_UnsavedDocument));
    ...
    const bool tab_just_unsaved = (flags & ImGuiTabItemFlags_UnsavedDocument) and !(tab->Flags & ImGuiTabItemFlags_UnsavedDocument);
    ...
    if (!is_tab_button)
    {
        if (tab_appearing and (tab_bar->Flags & ImGuiTabBarFlags_AutoSelectNewTabs) and tab_bar->NextSelectedTabId == 0)
            if (!tab_bar_appearing or tab_bar->SelectedTabId == 0)
                TabBarQueueFocus(tab_bar, tab); // New tabs gets activated
        if ((flags & ImGuiTabItemFlags_SetSelected) and (tab_bar->SelectedTabId != id)) // _SetSelected can only be passed on explicit tab bar
            TabBarQueueFocus(tab_bar, tab);
    }
    ...
    if (!tab_contents_visible and tab_bar->SelectedTabId == 0 and tab_bar_appearing)
        if (tab_bar->Tabs.Size == 1 and !(tab_bar->Flags & ImGuiTabBarFlags_AutoSelectNewTabs))
    ...
    {
        if (!g.DragDropActive and (tab_bar->Flags & ImGuiTabBarFlags_Reorderable))
        {
    ...
    const ImU32 tab_col = GetColorU32((held or hovered) ? ImGuiCol_TabHovered : tab_contents_visible ? (tab_bar_focused ? ImGuiCol_TabActive : ImGuiCol_TabUnfocusedActive) : (tab_bar_focused ? ImGuiCol_Tab : ImGuiCol_TabUnfocused));
    ...
    if (text_clipped and g.HoveredId == id and !held)
        if (!(tab_bar->Flags & ImGuiTabBarFlags_NoTooltip) and !(tab->Flags & ImGuiTabItemFlags_NoTooltip))
...
{
    ImGuiContext& g = *GImGui;
    bool is_within_manual_tab_bar = g.CurrentTabBar and !(g.CurrentTabBar->Flags & ImGuiTabBarFlags_DockNode);
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
    ...
    bool unsaved_marker_visible = (flags & ImGuiTabItemFlags_UnsavedDocument) != 0 and (button_pos.x + button_sz <= bb.Max.x);
    ...
    if (close_button_visible)
    {
        ...
        if (!(flags & ImGuiTabItemFlags_NoCloseWithMiddleMouseButton) and IsMouseClicked(2))
            close_button_pressed = true;
    }
