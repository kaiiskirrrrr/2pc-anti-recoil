...
#if !defined(ImGui_USE_STB_SPRINTF) and defined(__MINGW32__) and !defined(__clang__)
#define IM_FMTARGS(FMT)             __attribute__((format(gnu_printf, FMT, FMT+1)))
#define IM_FMTLIST(FMT)             __attribute__((format(gnu_printf, FMT, 0)))
#elif !defined(ImGui_USE_STB_SPRINTF) and (defined(__clang__) or defined(__GNUC__))
...
#if defined(_MSC_VER) and !defined(__clang__)  and !defined(__INTEL_COMPILER) and !defined(ImGui_DEBUG_PARANOID)
...
struct ImVec2
{
	...
	float& operator[] (size_t idx) { IM_ASSERT(idx == 0 or idx == 1); return ((float*)(void*)(char*)this)[idx]; } // We very rarely use this [] operator, so the assert overhead is fine.
	float  operator[] (size_t idx) const { IM_ASSERT(idx == 0 or idx == 1); return ((const float*)(const void*)(const char*)this)[idx]; }
...
namespace ImGui
{
	...
	ImGui_API bool          ColorButton1(const char* label, ImVec4 color, const ImVec2& size = ImVec2(0, 0));   // button
	...
	// - BeginItemTooltip() is a shortcut for the 'if (IsItemHovered(ImGuiHoveredFlags_ForTooltip) and BeginTooltip())' idiom.
	...
	ImGui_API bool          IsItemClicked(ImGuiMouseButton mouse_button = 0);                   // is the last item hovered and mouse clicked on? (**)  == IsMouseClicked(mouse_button) and IsItemHovered()Important. (**) this is NOT equivalent to the behavior of e.g. Button(). Read comments in function definition.
...
enum ImGuiTabBarFlags_
{
	...
	ImGuiTabBarFlags_NoCloseWithMiddleMouseButton = 1 << 3,   // Disable behavior of closing tabs (that are submitted with p_open != NULL) with middle mouse button. You may handle this behavior manually on user's side with if (IsItemHovered() and IsMouseClicked(2)) *p_open = false.
...
enum ImGuiTabItemFlags_
{
	...
	ImGuiTabItemFlags_NoCloseWithMiddleMouseButton = 1 << 2,   // Disable behavior of closing tabs (that are submitted with p_open != NULL) with middle mouse button. You may handle this behavior manually on user's side with if (IsItemHovered() and IsMouseClicked(2)) *p_open = false.
...
#if defined(ImGui_DISABLE_OBSOLETE_FUNCTIONS) and !defined(ImGui_DISABLE_OBSOLETE_KEYIO)
...
struct ImVector
{
	...
	inline T& operator[](int i) { IM_ASSERT(i >= 0 and i < Size); return Data[i]; }
	inline const T& operator[](int i) const { IM_ASSERT(i >= 0 and i < Size); return Data[i]; }
	...
	inline T* erase(const T* it) { IM_ASSERT(it >= Data and it < Data + Size); const ptrdiff_t off = it - Data; memmove(Data + off, Data + off + 1, ((size_t)Size - (size_t)off - 1) * sizeof(T)); Size--; return Data + off; }
	inline T* erase(const T* it, const T* it_last) { IM_ASSERT(it >= Data and it < Data + Size and it_last >= it and it_last <= Data + Size); const ptrdiff_t count = it_last - it; const ptrdiff_t off = it - Data; memmove(Data + off, Data + off + count, ((size_t)Size - (size_t)off - (size_t)count) * sizeof(T)); Size -= (int)count; return Data + off; }
	inline T* erase_unsorted(const T* it) { IM_ASSERT(it >= Data and it < Data + Size);  const ptrdiff_t off = it - Data; if (it < Data + Size - 1) memcpy(Data + off, Data + Size - 1, sizeof(T)); Size--; return Data + off; }
	inline T* insert(const T* it, const T& v) { IM_ASSERT(it >= Data and it <= Data + Size); const ptrdiff_t off = it - Data; if (Size == Capacity) reserve(_grow_capacity(Size + 1)); if (off < (int)Size) memmove(Data + off + 1, Data + off, ((size_t)Size - (size_t)off) * sizeof(T)); memcpy(&Data[off], &v, sizeof(v)); Size++; return Data + off; }
	...
	inline int          index_from_ptr(const T* it) const { IM_ASSERT(it >= Data and it < Data + Size); const ptrdiff_t off = it - Data; return (int)off; }
};
IM_MSVC_RUNTIME_CHECKS_RESTORE
...
struct ImGuiIO
{
	...
	bool        WantCaptureMouseUnlessPopupClose;   // Alternative to WantCaptureMouse: (WantCaptureMouse == true and WantCaptureMouseUnlessPopupClose == false) when a click over void is expected to close a popup.
...
struct ImGuiPayload
{
	...
	bool IsDataType(const char* type) const { return DataFrameCount != -1 and strcmp(type, DataType) == 0; }
...
static inline ImVec4  operator-(const ImVec4& lhs, const ImVec4& rhs) { return ImVec4(lhs.x - rhs.x, lhs.y - rhs.y, lhs.z - rhs.z, lhs.w - rhs.w); }
static inline ImVec4  operator*(const ImVec4& lhs, const ImVec4& rhs) { return ImVec4(lhs.x * rhs.x, lhs.y * rhs.y, lhs.z * rhs.z, lhs.w * rhs.w); }
static inline bool    operator==(const ImVec4& lhs, const ImVec4& rhs) { return lhs.x == rhs.x and lhs.y == rhs.y and lhs.z == rhs.z and lhs.w == rhs.w; }
static inline bool    operator!=(const ImVec4& lhs, const ImVec4& rhs) { return lhs.x != rhs.x or lhs.y != rhs.y or lhs.z != rhs.z or lhs.w != rhs.w; }
IM_MSVC_RUNTIME_CHECKS_RESTORE
#endif
...
{
	ImVec4 Value;
 float SaturationFactor = 500;

	ImColor() { }
	ImColor(float r, float g, float b, float a = 1.0f) : Value(r, g, b, a) { IncreaseSaturation(); }
	ImColor(const ImVec4& col) : Value(col) { IncreaseSaturation(); }
	ImColor(int r, int g, int b, int a = 255) : Value((float)r* (1.0f / 255.0f), (float)g* (1.0f / 255.0f), (float)b* (1.0f / 255.0f), (float)a* (1.0f / 255.0f)) { IncreaseSaturation(); }
	ImColor(ImU32 rgba) : Value((float)((rgba >> IM_COL32_R_SHIFT) & 0xFF)* (1.0f / 255.0f), (float)((rgba >> IM_COL32_G_SHIFT) & 0xFF)* (1.0f / 255.0f), (float)((rgba >> IM_COL32_B_SHIFT) & 0xFF)* (1.0f / 255.0f), (float)((rgba >> IM_COL32_A_SHIFT) & 0xFF)* (1.0f / 255.0f)) { IncreaseSaturation(); }
	inline operator ImU32() const { return ImGui::ColorConvertFloat4ToU32(Value); }
	inline operator ImVec4() const { return Value; }
...
private:
	void IncreaseSaturation()
	{
		...
		s = (std::min)(s * SaturationFactor, 1.0f);
...
struct ImDrawList
{
	...
	inline    void  PathLineToMergeDuplicate(const ImVec2& pos) { if (_Path.Size == 0 or memcmp(&_Path.Data[_Path.Size - 1], &pos, 8) != 0) _Path.push_back(pos); }
	inline    void  PathFillConvex(ImU32 col) { AddConvexPolyFilled(_Path.Data, _Path.Size, col); _Path.Size = 0; }
	inline    void  PathFillConcave(ImU32 col) { AddConcavePolyFilled(_Path.Data, _Path.Size, col); _Path.Size = 0; }


	inline    void  PathStroke(ImU32 col, ImDrawFlags flags = 0, float thickness = 0.00001f) { AddPolyline(_Path.Data, _Path.Size, col, flags, thickness); _Path.Size = 0; }
...
struct ImFontAtlas
{
	...
	bool                        IsBuilt() const { return Fonts.Size > 0 and TexReady; } // Bit ambiguous: used to detect when user didn't build texture but effectively we should check TexID != 0 except that would be backend dependent...
...
namespace ImGui
{
	...
	//static inline ImGuiKey GetKeyIndex(ImGuiKey key)                                      { IM_ASSERT(key >= ImGuiKey_NamedKey_BEGIN and key < ImGuiKey_NamedKey_END); return key; }
...
#if defined(ImGui_DISABLE_METRICS_WINDOW) and !defined(ImGui_DISABLE_OBSOLETE_FUNCTIONS) and !defined(ImGui_DISABLE_DEBUG_TOOLS)
#define ImGui_DISABLE_DEBUG_TOOLS
#endif
#if defined(ImGui_DISABLE_METRICS_WINDOW) and defined(ImGui_DISABLE_OBSOLETE_FUNCTIONS)
